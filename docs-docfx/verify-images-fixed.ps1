...
function Crawl-Site-For-Images {
    ...
    while ($urlsToVisit.Count -gt 0 -and $totalChecked -lt $MaxPages -and (Get-Date) -lt $endTime) {
        ...
        if ((Get-Date) -ge $endTime) {
            Write-Host "Timeout reached after $TimeoutSeconds seconds. Stopping image verification." -ForegroundColor Yellow
            break
        }
        
        # Check if we've reached the maximum number of pages
        if ($totalChecked -ge $MaxPages) {
            Write-Host "Maximum number of pages ($MaxPages) reached. Stopping image verification." -ForegroundColor Yellow
            break
        }
    }
...
Write-Host "Maximum pages to check: $MaxPagesToCheck, Timeout: $TimeoutSeconds seconds" -ForegroundColor Cyan
$results = Crawl-Site-For-Images -StartUrl $baseUrl -BaseUrl $baseUrl -MaxPages $MaxPagesToCheck -Timeout $TimeoutSeconds

# Output summary
Write-Host "`nImage verification complete!" -ForegroundColor Green
Write-Host "Pages checked: $($results.PagesChecked)" -ForegroundColor Cyan
