# Incremental script to verify and fix image references in markdown files
# This script processes files in batches and saves progress to avoid token limit issues
...
function GenerateSummaryReport {
    ...
    if ($fixIssues) {
        ...
        Write-Host "Issues not fixed (no alternative found): $($allIssues.Count - $fixedCount)"
    }
...
function Main {
    ...
    $allIssues = if ($continueFromLastRun) { $progress.AllIssues } else { @() }
    
    # Process files in batches
    while ($startIndex -lt $markdownFiles.Count) {
        $result = ProcessMarkdownBatch -markdownFiles $markdownFiles -startIndex $startIndex -batchSize $batchSize -fixIssues:$fixIssues -preferSnakeCase:$preferSnakeCase -progress $progress
        
        $batchIssues = $result.Issues
        $progress = $result.Progress
        
        # Add batch issues to all issues
        $allIssues += $batchIssues
        
        # Update progress with all issues
        $progress.AllIssues = $allIssues
        ...
        Write-Host "Total issues found so far: $($allIssues.Count)"
    ...
    GenerateDetailedReport -allIssues $allIssues
    
    # Export to CSV
    if ($csvReport) {
        ExportIssuesToCsv -allIssues $allIssues -csvFile $csvReport
    }
...
Write-Host "Image Reference Verification"
Write-Host "==========================="
...
Main -rootDir $rootDir -fixIssues:$fixIssues -preferSnakeCase:$preferSnakeCase -batchSize $batchSize -progressFile $progressFile -csvReport $csvReport -continueFromLastRun:$continueFromLastRun
